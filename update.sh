...
for optimized in debian alpine anolis test; do
...
dockerhublink="https://registry.hub.docker.com/r/jieme/postgis/tags?page=1&name="
githubrepolink="https://github.com/jieme/docker-postgis/blob/master"
...
for version in "${versions[@]}"; do
    ...
    then
        ...
        (
            ...
            echo "| [jieme/postgis:${version}](${dockerhublink}${version}) | [Dockerfile](${githubrepolink}/${version}/Dockerfile) | debian:${suite} | ${postgresVersion} | ${postgisDocSrc} |" >> _dockerlists_${optimized}.md
        )
    fi
    ...
    for variant in alpine anolis; do
        ...
        (
            ...
            if [ "$variant" == "anolis" ]; then
                cp -p Dockerfile.anolis.template initdb-postgis.sh update-postgis.sh "$version/$variant/"
                mv "$version/$variant/Dockerfile.anolis.template" "$version/$variant/Dockerfile"
                sed -i 's/%%TXT_AUTOGENERATED%%/'"$autogenerated"'/g; s/%%PG_MAJOR%%/'"$postgresVersion"'/g; s/%%POSTGIS_VERSION%%/'"$srcVersion"'/g' "$version/$variant/Dockerfile"
                echo "| [jieme/postgis:${version}-${variant}](${dockerhublink}${version}-${variant}) | [Dockerfile](${githubrepolink}/${version}/${variant}/Dockerfile) | openanolis/anolisos:8.2-x86_64 | ${postgresVersion} | ${postgisDocSrc} |" >> _dockerlists_${optimized}.md
            else
                ...
                echo "| [jieme/postgis:${version}-${variant}](${dockerhublink}${version}-${variant}) | [Dockerfile](${githubrepolink}/${version}/${variant}/Dockerfile) | alpine:${defaultAlpinenSuite} | ${postgresVersion} | ${postgisDocSrc} |" >> _dockerlists_${optimized}.md
            fi
        )
    done
done
